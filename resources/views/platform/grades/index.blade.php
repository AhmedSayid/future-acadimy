...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الصفوف الدراسية</h4><span
                    class="text-muted mt-1 tx-13 mr-2 mb-0">عرض الصفوف الدراسية</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
                ...
                <div class="card-header pb-0">
                    ...
                    <div class="col-sm-6 col-md-4 col-xl-3 mg-t-20">
                        <a class="modal-effect btn btn-outline-primary btn-block" data-effect="effect-fall"
                           data-toggle="modal" href="#modaldemo8">إضافة صف دراسي</a>
                        <button type="button"
    ...
    <div class="modal" id="editGradeModal" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                <form id="editGradeForm">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title">تعديل صف دراسي</h5>
    ...
    <div class="modal" id="modaldemo8" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">إضافة صف دراسي</h6>
...
@endsection
@section('js')
    @include('platform.layouts.get_data' , ['index_route' => url('platform/grades')])
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-edit', function (e) {
                ...
                $.ajax({
                    url: `{{ route("grades.get-grade-data", '') }}/${userId}`,
                    method: 'GET',
            ...
            $('#editGradeForm').on('submit', function (e) {
                ...
                let gradeId = $('#editGradeForm [name="id"]').val();

                $.ajax({
                    url: `{{ route("grades.update", '') }}/${gradeId}`,
                    method: 'POST',
                    data: formData,
                    success: function (response) {
                        if (response.key === 'success') {
            ...
            $(document).on('click', '.btn-delete', function (e) {
                ...
                if (confirm('هل أنت متأكد من حذف هذا الصف؟')) {
                    $.ajax({
                        url: `{{ route("grades.delete", ":id") }}`.replace(':id', gradeId),
                        method: 'DELETE',
                        headers: {
            ...
            $('#gradeForm').on('submit', function (e) {
                ...
                $.ajax({
                    url: '{{ route("grades.store") }}', // Adjust route as needed
                    method: 'POST',
                    data: formData,
                    dataType: 'json',
                    beforeSend: function () {
                        $('#saveGrade').html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>').attr('disabled', true);
                    },
                    success: function (response) {
                        if (response.key === 'success') {
                    ...
                    complete: function () {
                        $('#saveGrade').html('إضافة').attr('disabled', false);
                    }
                });
            });
        });
