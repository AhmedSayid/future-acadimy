<?php
$grades = \App\Models\Grade::with('subjects.teachers.user')->get();
...
<aside
    ...
    <div class="collapse navbar-collapse px-0 w-auto h-auto" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a data-bs-toggle="collapse" href="#componentsExamples" class="nav-link {{ (Str::contains(Request::path(), 'users') ? 'active' : '') }}" aria-controls="componentsExamples" role="button" aria-expanded="false">
                   <span class="avatar avatar-sm border-radius-md {{ (Str::contains(Request::path(), 'users') ? 'bg-gradient-info' : 'bg-white shadow') }}">
                        <svg class="{{ (Str::contains(Request::path(), 'users') ? 'text-white' : 'text-dark') }}" width="14px" height="12px" viewbox="0 0 46 42" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                ...
                <div class="collapse {{ (Str::contains(Request::path(), 'users') ? 'show' : '') }}" id="componentsExamples">
                    <ul class="nav ms-4 ps-3">
                        <li class="nav-item ">
                            <a class="nav-link {{ (Route::is('teachers') ? 'active' : '') }} {{ (Str::contains(Request::path(), 'teachers') ? 'active' : '')}}"
                               href="{{route('teachers')}}">
                                <span class="sidenav-mini-icon"> د </span>
                                <span class="sidenav-normal">الدكاتره والمدرسين</span>
                            </a>
                        </li>
                    </ul>
                </div>
